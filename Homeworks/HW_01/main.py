...
print(*nameList, type(nameList), sep='\n')      # Каждое значение на новой строке
print(*nameTuple, type(nameTuple))
print(*nameTuple2, type(nameTuple2), sep=', ')  # Каждое значение отделено запятой
print(*nameSet, type(nameSet))
print(*nameSet2, type(nameSet2))
print(*nameFrozenSet, type(nameFrozenSet))
print(str(nameDict).replace(",", ",\n"))        # Попытка вывода ключа-значения построчно

name_list_var = [nameString, nameInteger, nameFloat, nameBytes, nameList, nameTuple,
                 nameTuple2, nameSet, nameSet2, nameFrozenSet, nameDict]
for name in name_list_var:
    print(name, type(name))

# for a, b in nameDict.items():   # НЕУДАЧНЫЙ Вывод ключей и значений из словаря построчно
#     print(a)
#     for c, d in nameDict.items():
#         print('{}: {}'.format(c, d))
...
# 16) Создать переменную в которой надо присвоить остаток от деления этих переменнх Int.
# Вывести в консоль.
